...
export const signUpStart = (email, fullName, username, password,specialUser,constituency) => async (
  dispatch
) => {
  try {
    dispatch({ type: userTypes.SIGN_IN_START });
    const response = await registerUser(email, fullName, username, password,specialUser,constituency);
